...
def main():
    ...
    # Show help
    run_command(
        f"{python_path} -m src.icon_curator.cli.main --help",
        "Show CLI help"
    )
    
    # Show stats (will fail gracefully if no database)
    run_command(
        f"{python_path} -m src.icon_curator.cli.main stats",
        "Show database statistics"
    )
    
    # Show search help
    run_command(
        f"{python_path} -m src.icon_curator.cli.main search --help",
        "Show search command help"
    )
    
    # Show scrape help
    run_command(
        f"{python_path} -m src.icon_curator.cli.main scrape --help",
    ...
    print("3. Run: python -m src.icon_curator.cli.main scrape")
    print("4. Run: python -m src.icon_curator.cli.main search 'your query'")
